...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("bitengen"))
        return false;
...
{
    // Convert bitengen:// to bitengen:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bitengen://"))
    {
        uri.replace(0, 12, "bitengen:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Bitengen.lnk";
}
...
{
    return GetAutostartDir() / "bitengen.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Bitengen-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  bitengen-qt [" + tr("command-line options") + "]                     " + "\n";
